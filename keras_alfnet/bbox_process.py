...
def get_target_1st(all_anchors, regr_layer, img_data, C,roi_stride=10,igthre=0.5,posthre=0.7,negthre=0.5):
    ...
    for i in range(regr_layer.shape[0]):
        ...
        if len(ignoreareas) > 0:
            ignore_overlap = iou(np.ascontiguousarray(proposals[:, :4], dtype=np.float), np.ascontiguousarray(ignoreareas, dtype=np.float))
        ...
        if num_bboxes > 0:
            ...
            valid_rpn_regr[fg_inds, :] = compute_targets(anchor_box, gt_box, C.classifier_regr_std, std=True)
