...
def calc_target_multilayer(C, img_data, anchors, igthre=0.5, posthre=0.5, negthre=0.3):
    ...
    if len(ignoreareas) > 0:
        # ignore_overlap = box_op(np.ascontiguousarray(all_anchors[:, :4], dtype=np.float), np.ascontiguousarray(ignoreareas, dtype=np.float))
        ignore_overlap = iou(np.ascontiguousarray(all_anchors[:, :4], dtype=np.float), np.ascontiguousarray(ignoreareas, dtype=np.float))
    ...
    if num_bboxes>0:
        # valid_overlap = bbox_overlaps(np.ascontiguousarray(valid_anchors, dtype=np.float), np.ascontiguousarray(gta, dtype=np.float))
        valid_overlap = iou(np.ascontiguousarray(valid_anchors, dtype=np.float), np.ascontiguousarray(gta, dtype=np.float))
